...
import { Truck, PlusCircle, User, Scale, Pencil, Trash2, Lock, Unlock, Phone, CalendarDays, ShieldCheck } from "lucide-react";
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-200 pb-6">
        <div>
          ...
          <p className="text-slate-500 text-xs font-medium">
            إدارة القوى العاملة، تتبع الشاحنات، ومراقبة الحالات التشغيلية لسائقي الشلال
          </p>
        </div>
        {role === "admin" && (
          <Link href="/drivers/new">
            <Button
              size="lg"
              className="w-full md:w-auto h-13 px-8 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-black shadow-xl shadow-blue-600/20 gap-3 transition-all active:scale-95"
            >
        ...
        {isLoading ? (
          ...
          drivers.map((driver) => {
            ...
            return (
              <Card
                key={driver.id}
                className={`group border-none shadow-xl shadow-slate-200/50 rounded-3xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 ${
                  isFrozen ? "bg-slate-50 opacity-90" : "bg-white"
                }`}
              >
                {/* Status Bar */}
                ...
                <div className="p-6">
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className={`p-4 rounded-2xl shadow-inner ${
                          isFrozen ? "bg-slate-200 text-slate-400" : "bg-blue-50 text-blue-600"
                        }`}
                      >
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {isFrozen && (
                            <Badge className="bg-amber-100 text-amber-700 border-none rounded-lg text-[9px] font-black uppercase tracking-widest">
                              {t("frozen")}
                            </Badge>
                          )}
                        </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-6">
                    ...
                    <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100/50 flex flex-col items-center justify-center text-center">
                      ...
                      <span className="text-xs font-black text-slate-900">
                        {format(new Date(driver.createdAt), "MMM yyyy")}
                      </span>
                    </div>
                  </div>
                  ...
                  {driver.phone && (
                    <div className="flex items-center gap-3 px-4 py-3 bg-blue-50/30 border border-blue-100/50 rounded-2xl mb-6">
                      ...
                      <span className="text-xs font-black text-blue-900 font-mono tracking-tighter" dir="ltr">
                        {driver.phone}
                      </span>
                    </div>
                  )}
                    ...
                    <div className="pt-5 border-t border-slate-100 space-y-3">
                      <div className="flex gap-2">
                        <Link href={`/drivers/${driver.id}/settle`} className="flex-1">
                          <Button
                            variant="outline"
                            className="w-full h-10 rounded-xl bg-white border-slate-200 text-slate-700 font-black text-xs gap-2 hover:bg-slate-900 hover:text-white transition-all shadow-sm"
                          >
                        ...
                        <Link href={`/drivers/${driver.id}/edit`} className="flex-1">
                          <Button
                            variant="outline"
                            className="w-full h-10 rounded-xl bg-white border-slate-200 text-slate-700 font-black text-xs gap-2 hover:bg-slate-50 shadow-sm"
                          >
                      ...
                      <div className="flex gap-2">
                        <Button
                          variant="ghost"
                          className={`flex-1 h-10 rounded-xl font-black text-[10px] gap-2 transition-all ${
                            isFrozen
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button
                              variant="ghost"
                              className="h-10 w-12 rounded-xl p-0 bg-red-50 text-red-600 hover:bg-red-100 transition-colors"
                            >
                          ...
                          <AlertDialogContent className="rounded-3xl border-none shadow-2xl">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription className="font-medium text-slate-500">
                                سيتم حذف السائق (<strong>{driver.name}</strong>) نهائياً من النظام. لا يمكن
                                التراجع عن هذا الإجراء وسيؤثر على سجلات التقارير المرتبطة به.
          ...
          <div className="col-span-full py-20 text-center border-4 border-dashed border-slate-100 rounded-[40px] bg-slate-50/30">
            <User className="h-20 w-20 mx-auto text-slate-200 mb-6" />
            <p className="text-slate-400 font-black text-lg">{t("noDrivers")}</p>
            <p className="text-slate-300 text-sm font-medium mt-2">لم يتم تسجيل أي سائق في النظام بعد</p>
            {role === "admin" && (
              <Link href="/drivers/new">
                <Button className="mt-6 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-black gap-2 shadow-lg shadow-blue-600/20">
